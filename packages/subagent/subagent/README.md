...
`SubagentStartRequest.workspaceCwd` gives a child an explicit absolute working directory for one run. A provider must advertise `capabilities.workspaceCwd: true` and validate the directory before starting; the service rejects the request when that capability is absent. For out-of-process providers, this caller-supplied directory takes precedence over a configured provider `cwd`, which in turn takes precedence over the parent session's cwd. In-process providers create the child session with the requested cwd. This contract selects a working directory; it is not an OS sandbox, permission policy, or filesystem boundary.
