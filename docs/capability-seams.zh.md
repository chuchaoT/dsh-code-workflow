...
flowchart LR
  ...
  pkg_experimental_autodev["experimental-autodev"]
  svc_autodev["ctx.autodev<br/>Resumable software-engineering workflow"]
  ...
  pkg_experimental_autodev --> svc_autodev
...
| `ctx.autodev` | `service` | [`experimental-autodev`](../packages/experimental/autodev) | - | - | - | 管理持久 Run、隔离 Worktree Attempt、协议规范化、验证 Evidence、项目作用域知识和显式晋级；复用 DSH 已加载的 Provider，并确保 Client 操作受 Host Remote 管理。 |
